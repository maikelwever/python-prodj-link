...
def update_clients(p, client_win):
  try:
    client_win.clear()
...
def main():
  ...
  p = ProDj()
  p.set_client_keepalive_callback(lambda n: update_clients(p, client_win))
  p.set_client_change_callback(lambda n: update_clients(p, client_win))

  update_clients(p, client_win)
